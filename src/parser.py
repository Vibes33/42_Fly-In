...
class MapParser:
    ...
    def parse(self) -> MapData:
        ...
        while self.line_num < len(lines):
            ...
            if not line.startswith('nb_drones:'):
                raise ParsingError(
                    "First line must define nb_drones", self.line_num
                )
        ...
        for line in lines[self.line_num:]:
            ...
            if any(line.startswith(prefix) for prefix in prefixes):
                ...
                raise ParsingError(
                    f"Unknown line format: {line}", self.line_num
                )
    ...
    def _extract_metadata(self, line: str) -> Tuple[str, Dict[str, str]]:
        ...
        if match:
            ...
            for pair in pairs:
                if '=' not in pair:
                    raise ParsingError(
                        f"Invalid metadata format: '{pair}'", self.line_num
                    )
    ...
    def _parse_zone(self, line: str) -> None:
        ...
        if '-' in name:
            raise ParsingError(
                f"Zone name '{name}' contains forbidden dash.",
                self.line_num,
            )
        ...
        try:
            ...
            raise ParsingError(
                f"Coordinates must be integers for zone {name}",
                self.line_num,
            )
        ...
        if zone_type not in self.valid_zone_types:
            raise ParsingError(
                f"Invalid zone type: {zone_type}", self.line_num
            )
        ...
        if prefix == 'start_hub:':
            if self.map_data.start_hub is not None:
                raise ParsingError(
                    "Multiple start_hub defined", self.line_num
                )
            self.map_data.start_hub = name
        elif prefix == 'end_hub:':
            if self.map_data.end_hub is not None:
                raise ParsingError(
                    "Multiple end_hub defined", self.line_num
                )
    ...
    def _parse_connection(self, line: str) -> None:
        ...
        if not main_line.startswith(prefix):
            raise ParsingError(
                "Invalid connection prefix", self.line_num
            )
        ...
        if len(nodes) != 2:
            raise ParsingError(
                "Invalid connection format. Expected node1-node2",
                self.line_num,
            )
        ...
        if (
            n1 not in self.map_data.zones
            or n2 not in self.map_data.zones
        ):
            raise ParsingError(
                f"Unknown zone in connection: {n1}-{n2}",
                self.line_num,
            )

        # check for duplicate connection
        if (
            n1 in self.map_data.adjacency_list
            and n2 in self.map_data.adjacency_list[n1]
        ):
            raise ParsingError(
                f"Duplicate connection: {n1}-{n2}", self.line_num
            )
        ...
        try:
            ...
            raise ParsingError(
                f"Invalid max_link_capacity: {max_cap_str}",
                self.line_num,
            )

        # Add to model
        self.map_data.connections.append(
            Connection(n1, n2, max_link_cap)
        )
